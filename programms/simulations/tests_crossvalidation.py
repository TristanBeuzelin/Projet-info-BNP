...
nb_folds = 5
grid = GridSearchCV(Model, tuned_parameters, cv=nb_folds, refit=True, verbose=3)
...
nb_folds = 5
grid = GridSearchCV(Model, tuned_parameters, cv=nb_folds, refit=True, verbose=3, scoring='explained_variance')
grid.fit(prices_learn, IPC_learn.ravel())
scores = grid.cv_results_['mean_test_score']
scores_std = grid.cv_results_['std_test_score']
Predict_test = grid.best_estimator_.predict(price_test.T)
